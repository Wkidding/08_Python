...
但这样返回的是容器对象本身，调用方需要按容器类型的方式访问，不如元组解包方便。一般约定：当需要返回固定个数、语义不同的值时，优先使用元组；当值的数量可变或语义相同，则使用列表或其他集合。

---

### 5. 类型注解

为了增强代码可读性和静态检查，可以为返回值添加类型注解。对于多个返回值，通常使用 `tuple` 类型（Python 3.9+ 可直接用内置泛型）：
...
如果使用较旧的 Python 版本，需要从 `typing` 导入 `Tuple`：
...
如果返回的元组长度可变，可以使用 `tuple[Any, ...]` 或 `Tuple[Any, ...]`。

...
t4 = tuple("Python")    # ('P', 'y', 't', 'h', 'o', 'n')

# 2、空元组
t5=()
print(f'空元组：{t5},类型为：{type(t5)}')

# 3、单元素元组（必须加逗号）
t6_1 = (42,)      # 正确，类型为 tuple
t6_2 = (42)       # 注意：这是整数，不是元组

# 4、省略括号（逗号分隔）
t7 = 1, 2, 3    # 等同于 (1, 2, 3)，类型为 tuple
```

### 不可变性的含义

元组一旦创建，其**长度固定**，且**元素不可重新赋值**。但如果元素是可变对象（如列表），该对象本身可以被修改。元组也没有 `append`、`remove`、`pop`、`insert` 等修改自身的方法。
...
# 总结：
# (1) 元组中存储的其实是对象的引用（指向对象的指针）。
# (2) 元组不可变，指的是这些引用本身不能被修改，而不是引用所指向的对象不能被修改。
x = [1, 2]
t = (x, 3)

print(t)              # ([1, 2], 3)
print(id(t[0]))       # 输出列表 x 的内存地址

x.append(4)           # 修改了列表 x 的内容
print(t)              # ([1, 2, 4], 3)
print(id(t[0]))       # 地址没有变化，说明元组中存的引用没变
```
...
实际上元组是**浅层不可变**。元素引用不能变，但引用指向的对象可以变。

如果元组中包含可变对象（如列表、字典、集合、自定义对象等），这些可变对象的内容可以被修改。

```python
t = (1, [2, 3], {'a': 4})

t[1].append(100)       # 合法：修改元组中的列表
t[2]['b'] = 200        # 合法：修改元组中的字典

print(t)               # (1, [2, 3, 100], {'a': 4, 'b': 200})
```

但如果尝试替换元组中的元素，哪怕替换成完全相同的值，也不允许：

```python
t = ([1, 2], [3, 4])
t[0] = [1, 2]   # TypeError，即使值一样，引用变了也不允许
```
...
# 1、含义：把第一个元素单独给 first，剩下的所有元素都打包给 rest。
first, *rest = (1, 2, 3, 4)
print(first, rest)  # 1 [2, 3, 4]

# 2、含义：把最后一个元素单独给 last，前面的所有元素都打包给 start。
*start, last = (1, 2, 3, 4)
print(start, last)  # [1, 2, 3] 4 

# 3、含义：取第一个给 first，取最后一个给 last，中间剩余的所有元素（无论多少个）都打包给 middle。
first, *middle, last = (1, 2, 3, 4, 5)
print(first, middle, last)  # 1 [2, 3, 4] 5 

# 忽略某些值（使用 _）
_, b, _ = (1, 2, 3)
print(b)            # 2，相当于把下划线位置上的值都忽略
```
...
Python 中的序列类型：内容连续、有序，可使用下标索引的一类数据容器（list, tuple, str, range, bytes 等都属于序列）共享一组通用操作。以下操作适用于所有序列。

序列的典型特征就是
    （1）有序且可用下标索引   ------list、tuple、str，均满足这个要求
    （2）序列支持切片        ------list、tuple、str，均支持切片操作
...
语法：序列[起始下标:结束下标:步长]
含义：表示从序列中，从指定位置开始，依次取出元素，到指定位置结束，得到一个新序列：（即从一个序列中，取出一个子序列）
...
注意：
    (1)这个操作对列表、元组、字符串是通用的
    (2)此操作不会影响序列本身，而是会得到一个新的序列（列表、元组、字符串）
    (3)起始位置，结束位置，步长（正反序）都是可以自行控制的
...
new_list1 = my_list[1:4:1]     # 下标1开始，下标4（不含）结束，步长1
print(new_list1)               # 结果：[2, 3, 4]

new_list2 = my_list[:4:1]      # 从头开始，下标4（不含）结束，步长1
print(new_list2)               # 结果：[1, 2, 3, 4]

new_list3 = my_list[2::1]      # 下标2开始，到最后结束，步长1
print(new_list3)               # 结果：[3, 4, 5]

new_list4 = my_list[::1]      # 从头开始，到最后结束，步长1
print(new_list4)              # 结果：[1, 2, 3, 4, 5]

new_list5 = my_list[::2]      # 从头开始，到最后结束，步长2
print(new_list5)              # 结果：[1, 3, 5]

new_list6 = my_list[::-1]     # 从最后开始，到头结束，相当于反向取
print(new_list6)              # 结果：[5, 4, 3, 2, 1]

new_list6 = my_list[::-2]     # 从最后开始，到头结束，隔一个取
print(new_list6)              # 结果：[5, 3, 1]

new_list6 = my_list[-1:-5:-1] # 从最后开始，到-5位置结束
print(new_list6)              # 结果：[5, 4, 3, 2]
...
=============================切片作业====================
"""
work_str="zyx,987654321,nohtyp"
# 得到正序的1---9 字符串
res_str1 = work_str.split(',')[1][::-1]
print(f'第一种方式：结果为：{res_str1}')

res_str2 = work_str[::-1].split(',')[1]
print(f'第二种方式：结果为：{res_str2}')

res_str3=work_str.split(",")[1].replace(","," ")[::-1]
print(f'第三种方式：结果为：{res_str3}')

res_str4 = work_str[-8:-17:-1]
print(f'第四种方式：结果为：{res_str4}')

list_name = ["a", "b","c","d","e","f"]
print("取前三个名字：{}".format(list_name[0:3:1]))
new_list7 = list_name[-1:-4:-1]
print("取后三个名字，并按照原有顺序排列：{}".format(new_list7[::-1]))

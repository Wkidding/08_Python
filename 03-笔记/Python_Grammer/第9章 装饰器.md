...
![image-20260910074847728](images/image-20260910074847728.png)
...
`@property` 装饰器用于将类的方法转换为**只读属性**，使得我们可以像访问属性一样调用方法，而不需要显式使用括号。它通常用于实现**getter** 方法，使得外部代码可以像访问普通属性一样访问计算值或受保护的属性。

-   `@property` 装饰的方法称为**getter**，它必须返回一个值。
-   可以通过 `@属性名.setter` 和 `@属性名.deleter` 定义对应的**setter** 和 **deleter** 方法，从而实现属性的写入和删除操作。
-   使用 `@property` 可以实现**封装**，隐藏内部实现细节，同时保持简洁的访问语法。
...
![image-20260910205522729](images/image-20260910205522729.png)

#### 4.4 `@property` 的优势

-   **简洁的访问语法**：外部代码使用 `obj.attr` 而不是 `obj.get_attr()`。
-   **数据验证**：在 setter 中可以添加验证逻辑，防止非法数据。
-   **计算属性**：属性值可以根据其他属性动态计算（如 `area`），并且看起来像普通属性。
-   **向后兼容**：如果最初使用普通属性，后来需要添加逻辑，可以将其改为 `@property` 而不影响已有代码。

#### 4.5 实现原理

`@property` 是一个内置的**描述符**（descriptor）。描述符是实现了 `__get__`、`__set__` 或 `__delete__` 方法的对象。`property` 类实现了这些方法，因此可以控制属性的访问。当我们在类中定义 `@property` 时，实际上是创建了一个 `property` 对象，并将其赋值给类属性。访问该属性时，Python 会调用 `property` 对象的 `__get__` 方法，从而执行 getter 函数。

### 5. 三者对比总结

| 装饰器          | 用途                                         | 第一个参数 | 自动传入参数       | 访问实例属性 | 访问类属性       | 典型场景                       |
| :-------------- | :------------------------------------------- | :--------- | :----------------- | :----------- | :--------------- | :----------------------------- |
| `@staticmethod` | 与类相关的工具函数，不依赖类或实例状态       | 无         | 无                 | 否           | 否（除非显式）   | 数学工具、辅助函数             |
| `@classmethod`  | 操作类状态或提供备选构造函数                 | `cls`      | 类对象             | 否           | 是               | 工厂方法、修改类变量、继承相关 |
| `@property`     | 将方法伪装成属性，提供 getter/setter/deleter | `self`     | 实例对象（getter） | 是           | 是（通过`self`） | 数据封装、计算属性、属性验证   |

**关键区别**：

-   `@staticmethod` 和 `@classmethod` 定义的是**方法**，调用时需要括号（如 `obj.method()`）；而 `@property` 定义的是**属性**，访问时不需要括号（如 `obj.attr`）。
-   `@classmethod` 与继承紧密相关，因为 `cls` 是动态绑定的，而 `@staticmethod` 不具备这种特性。
-   `@property` 主要用于实例属性的封装，通常配合私有属性（如 `_x`）使用。
...
类方法中的 `cls` 是实际调用时的类，因此子类调用时 `cls` 为子类，从而访问子类的属性。

![image-20260910210047025](images/image-20260910210047025.png)



#### 6.2 静态方法不能访问类属性吗？

严格来说，静态方法可以通过类名访问类属性，但这样会硬编码类名，不利于继承。例如：
...
class MyClass:
    ...
    @staticmethod
    def get_attr():
        return MyClass.attr  # 硬编码类名
```

如果子类继承并覆盖 `attr`，静态方法仍然返回父类的 `attr`，这通常不是期望的行为。因此，静态方法一般不应依赖于类状态。



#### 6.3 `@property` 与私有属性命名约定

Python 没有真正的私有属性，但习惯上使用单下划线 `_attr` 表示“受保护”的属性，提示外部不要直接访问。`@property` 通常用于控制对这些属性的访问，同时提供更安全的接口。

#### 6.4 只读属性

只定义 `@property` 而不定义 setter，即可创建只读属性。尝试赋值时会引发 `AttributeError`。



### 7. 总结

-   `@staticmethod`：定义与类相关但不依赖类或实例状态的工具方法，相当于在类命名空间中的普通函数。
-   `@classmethod`：定义操作类本身的方法，第一个参数为 `cls`，常用于工厂模式、修改类变量以及需要动态绑定类对象的场景。
-   `@property`：将方法转化为属性，实现 getter/setter/deleter，提供简洁的访问语法和封装能力。
...
在开发中，我们经常需要记录函数的调用情况，比如：谁被调用了、传入了什么参数、返回了什么结果、是否发生异常。如果每个函数都手动写 `print` 或 `logging`，代码会非常冗余。使用装饰器可以统一处理日志，做到**业务逻辑与日志逻辑分离**。
...
def log_call(func):
    ...
    def wrapper(*args, **kwargs):
        # 记录函数开始调用
        logging.info(f"调用函数: {func.__name__}, 位置参数: {args}, 关键字参数: {kwargs}")
        try:
            result = func(*args, **kwargs)
            logging.info(f"函数 {func.__name__} 返回: {result}")
            return result
        except Exception as e:
            logging.error(f"函数 {func.__name__} 抛出异常: {e}")
            raise  # 重新抛出异常，不吞掉
    return wrapper
...
![image-20260910210510150](images/image-20260910210510150.png)

#### 1.3 代码详解

| 代码                             | 说明                                                         |
| :------------------------------- | :----------------------------------------------------------- |
| `@wraps(func)`                   | 保留原函数的元数据（`__name__`、`__doc__` 等），否则 `divide.__name__` 会变成 `wrapper` |
| `*args, **kwargs`                | 接收任意数量和类型的位置参数与关键字参数，使装饰器通用       |
| `logging.info(...)`              | 记录调用前的信息，包括函数名和参数                           |
| `result = func(*args, **kwargs)` | 调用原函数，保存返回值                                       |
| `logging.info(...)`              | 记录返回值                                                   |
| `except Exception as e`          | 捕获异常并记录                                               |
| `raise`                          | **关键**：记录完异常后必须重新抛出，否则异常会被吞掉，调用者无法感知 |
| `return wrapper`                 | 返回包装函数，替换原函数                                     |

#### 1.4 进阶：支持自定义日志前缀（带参数的装饰器）
...
def log_with_prefix(prefix):
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            logging.info(f"[{prefix}] 开始执行 {func.__name__}")
            result = func(*args, **kwargs)
            logging.info(f"[{prefix}] {func.__name__} 执行完毕")
            return result
        return wrapper
    return decorator

@log_with_prefix("USER_MODULE")
...
![image-20260910210700798](images/image-20260910210700798.png)

**结构说明**：带参数的装饰器是**三层嵌套**：

1.  最外层 `log_with_prefix(prefix)`：接收装饰器参数。
2.  中间层 `decorator(func)`：接收被装饰函数。
3.  最内层 `wrapper(*args, **kwargs)`：接收调用时参数。
...
性能分析是开发中的重要环节。我们需要知道某个函数执行了多长时间，以便定位性能瓶颈。计时装饰器可以无侵入地为函数添加计时功能。
...
![image-20260910210936993](images/image-20260910210936993.png)

#### 2.3 代码详解

| 代码                    | 说明                                                         |
| :---------------------- | :----------------------------------------------------------- |
| `time.perf_counter()`   | 返回性能计数器的值（以秒为单位），精度最高，适合测量短时间间隔。不能用 `time.time()` 代替，后者受系统时钟调整影响 |
| `start` / `end`         | 分别在调用前后记录时间点                                     |
| `elapsed = end - start` | 计算耗时                                                     |
| `{elapsed:.6f}`         | 格式化为保留 6 位小数                                        |

#### 2.4 进阶：累计统计（类装饰器实现）

如果需要记录**总耗时**、**调用次数**、**平均耗时**，用类装饰器保存状态更方便：
...
class Timer:
    def __init__(self, func):
        update_wrapper(self, func)   # 保留原函数元数据
        self.func = func
        self.total_time = 0.0
        self.calls = 0
    ...
    def __call__(self, *args, **kwargs):
        ...
        print(f"{self.func.__name__} 第 {self.calls} 次调用，"
              f"本次耗时 {elapsed:.6f}s，"
              f"累计 {self.total_time:.6f}s，"
              f"平均 {self.total_time / self.calls:.6f}s")
        return result
...
![image-20260910211058569](images/image-20260910211058569.png)

#### 2.5 代码详解

| 代码                             | 说明                                                         |
| :------------------------------- | :----------------------------------------------------------- |
| `update_wrapper(self, func)`     | 把 `func` 的元数据复制到 `self` 上，等价于函数装饰器里的 `@wraps(func)` |
| `self.total_time` / `self.calls` | 用实例属性保存累计状态，这是类装饰器的优势                   |
| `__call__`                       | 让实例可被调用，触发时机是函数被调用时                       |
| 累加逻辑                         | 每次调用后更新总时间和调用次数                               |
...
对于计算密集型或 I/O 密集型的函数，如果相同参数被多次调用，重复计算会浪费资源。缓存装饰器可以**记住**函数的返回值，下次用相同参数调用时直接返回缓存结果，避免重复计算。
...
def memoize(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        print(f"缓存未命中，计算结果: {func.__name__}{args}")
        result = func(*args, **kwargs)
        cache[key] = result
        return result
...
![image-20260910211259596](images/image-20260910211259596.png)

#### 3.3 代码详解

| 代码                                          | 说明                                                         |
| :-------------------------------------------- | :----------------------------------------------------------- |
| `cache = {}`                                  | 定义在 `wrapper` 外部的字典，被 `wrapper` 闭包引用，因此能持久保存缓存 |
| `key = (args, tuple(sorted(kwargs.items())))` | 用位置参数和关键字参数构造哈希键。`sorted` 保证关键字参数顺序不同但内容相同也能命中 |
| `if key in cache`                             | 检查缓存是否存在                                             |
| `cache[key] = result`                         | 写入缓存                                                     |
| 递归调用 `fib`                                | 因为 `fib` 已被替换为 `wrapper`，递归调用也会走缓存逻辑，大幅提升效率 |

#### 3.4 使用标准库 `functools.lru_cache`

Python 内置了 `functools.lru_cache`，它提供了更完善的缓存功能（支持最大容量限制、缓存命中统计等）。
...
print(fib(100))                  # 瞬间返回
print(fib.cache_info())          # 查看缓存统计
fib.cache_clear()                # 清空缓存
```

![image-20260910211339840](images/image-20260910211339840.png)

#### 3.5 `lru_cache` 详解

| 参数/方法       | 说明                                                         |
| :-------------- | :----------------------------------------------------------- |
| `maxsize`       | 缓存的最大条目数。`None` 表示无限制；设为 `2` 的幂时性能最佳 |
| `cache_info()`  | 返回命名元组 `CacheInfo(hits, misses, maxsize, currsize)`    |
| `cache_clear()` | 清空缓存                                                     |
| LRU 策略        | Least Recently Used，缓存满时淘汰最久未使用的条目            |
| 要求            | 被装饰函数的参数必须**可哈希**（hashable），否则会报错       |

#### 3.6 缓存装饰器注意事项

1.  **参数必须可哈希**：列表、字典等可变类型不能作为参数。
2.  **缓存会占用内存**：无限制缓存可能导致内存泄漏，建议设置 `maxsize`。
3.  **函数副作用**：如果函数有副作用（如写文件、发请求），缓存可能导致副作用不执行。
4.  **实例方法**：装饰实例方法时，`self` 也会成为缓存键的一部分，可能导致对象无法被回收（内存泄漏），需谨慎使用。
...
在 Web 开发或后台系统中，不同的用户角色拥有不同的操作权限。权限装饰器可以在函数执行前检查当前用户是否有权限，无权限则拒绝执行。这是装饰器在**横切关注点**（cross-cutting concern）中的典型应用。
...
# 测试
delete_user(123)     # 成功（admin）
view_profile()       # 成功（user）
view_logs()          # 成功（admin）

# 模拟切换到普通用户
current_user["role"] = "user"
delete_user(456)     # 抛出 PermissionError
```

![image-20260910211612082](images/image-20260910211612082.png)

#### 4.3 代码详解

| 代码                               | 说明                                                         |
| :--------------------------------- | :----------------------------------------------------------- |
| `current_user`                     | 模拟当前登录用户信息，实际项目中通常从 session、JWT token 或请求上下文中获取 |
| `require_role(role)`               | 外层函数接收所需角色作为参数                                 |
| `decorator(func)`                  | 中间层接收被装饰函数                                         |
| `wrapper(*args, **kwargs)`         | 内层在调用前检查权限                                         |
| `current_user.get("role") != role` | 比较当前用户角色与所需角色                                   |
| `raise PermissionError(...)`       | 权限不足时抛出异常，阻止函数执行                             |
| `return func(*args, **kwargs)`     | 权限通过则正常执行                                           |
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if current_user.get("role") not in roles:
                raise PermissionError(f"需要以下角色之一: {roles}")
            return func(*args, **kwargs)
        return wrapper
    return decorator
...
# 模拟登录
current_user = {"name": "Bob", "role": "manager"}
approve_request(1001)   # 成功

current_user = {"name": "Eve", "role": "guest"}
approve_request(1002)   # PermissionError
```

![image-20260910211715397](images/image-20260910211715397.png)
...
```python
approve_request = login_required(require_any_role("admin", "manager")(approve_request))
```

**执行顺序**（调用时）：

1.  最外层 `login_required` 的 `wrapper` 先执行 → 检查登录。
2.  若通过，调用内层 `require_any_role` 的 `wrapper` → 检查角色。
3.  若通过，执行原函数 `approve_request`。
...
则先检查角色，但此时 `current_user` 可能为 `None`，会导致 `AttributeError`（因为 `None.get` 不存在）。所以**登录检查应该放在最外层**。

#### 4.6 权限装饰器的设计要点

1.  **异常设计**：使用 `PermissionError` 或自定义异常，便于上层统一处理。
2.  **上下文获取**：实际项目中，当前用户信息通常从 `request`、`session`、`token` 中获取，而非全局变量。可以结合 `contextvars` 或依赖注入。
3.  **装饰器顺序**：登录检查应在外层，角色/权限检查在内层。
4.  **可组合性**：支持多角色、多权限的组合，提高灵活性。
5.  **日志记录**：权限校验失败时应记录日志，便于安全审计。